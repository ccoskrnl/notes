...
| 特性 | May分析 | Must分析 |
|------|--------|---------|
| **目标** | 只要在**某一条**路径上成立，就认为它成立 | 识别必然发生的情况 |
| **近似方向** | 过度近似(over-approximation) | 不足近似(under-approximation) |
| **安全性** | 包含所有可能情况，**不能漏报** | 只包含必然发生情况，**不能错报** |
| **精度** | 可能包含假阳性 | 可能包含假阴性 |
| **适用场景** | 存在性问题 | 保证性问题 |
| **初始值** | 入口/出口点：最精确值(top) | 入口/出口点：最精确值(top) |
| **安全值** | 其他点：最保守值(bottom) | 其他点：最保守值(bottom) |
| **合并操作** | 并集(∪)或类似 | 交集(∩)或类似 |
| **典型例子** | 指针分析、到达定值分析 | 可用表达式分析、活跃变量分析（某些场景下） |

 **结果**

May分析最终报告的结果集合往往比实际情况**大**（Over-approximation）。

-   **真阳性**：确实可能发生的情况。
-   **假阳性**：分析说“可能发生”，但实际上由于分支条件限制，永远不会发生的情况。

Must分析最终报告的结果集合往往比实际情况**小**（Under-approximation）。

-   **真阴性**：确实不是在所有路径都成立的情况。
-   **假阴性**：实际上在所有运行路径中都成立，但由于分析器不够聪明，无法证明其中某条复杂路径也成立，所以报告了“不成立”。
...
不失一般性，我们假设现在感兴趣的数据流框架是一个前向的数据流问题。考虑一个基本块 $B$ 的入口点。求理想解的第一步是要找到**从程序入口到达 $B$ 的开头的所有可能的执行路径**。只有当程序的某次执行能够准确地沿着某条路径进行，这条路径才被称为“可能的” （**程序在实际执行中，所有真实可能走到的路径所产生的数据流事实的精确集合**）。理想的求解方法将计算每个可能路径尾端的数据流值，并对这些数据流值应用 $\wedge$(交汇) 运算得到它们的最大下界。那么，程序的任何执行都不可能在程序点上产生一个更小的数据流值（单调性）。另外，这个界限还是紧致：根据流图中到达 $B$ 的所有可能路径计算得到的数据流值的集合的最大下界不可能变得更大。
...
$$
这个理想结果是我们最希望的结果（故称作“理想”解），也就是能为我们提供最大程度且安全的优化的结果，即我们可以根据这个解来为程序做到最大程序并且维持程序原语义并且安全的优化。但现实是，它是**不可计算的**。因为要在静态分析阶段预测出程序到底会走哪些分支（比如 `if (复杂条件)`），本质上是一个不可判定问题（停机问题）。

按照问题中数据流框架的格理论偏序关系 $\preceq$ ，我们有下面结论：
...
**MOP(Meet-Over-All-Paths) 解**

由于寻找所有可能的执行路径是一个不可判定问题，因此，我们必须使用近似的方法。在数据流抽象中，假设流图中的每条路径都可能被执行，即假定控制流图 (CFG) 中的**所有物理路径都是可能执行的**。因此，我们可以用如下方法定义 $B$ 的基于路径交汇运算的解(MOP解)
$$
...
这是我们实际工程中使用的算法（如基于 IN/OUT 集合的方程求解）。它不穷举路径，而是**在每个节点的交汇处（Merge Point）立刻进行数据状态的合并**，然后拿着合并后（丢失了部分上下文）的状态继续往下算，直到所有节点的数据趋于稳定（达到不动点）。
...
迭代算法会在控制流汇聚的地方（$B_4$ 的入口处）立刻执行合并操作（IN 集合的计算）。MOP 会分别完整地走完左边和右边的路径，然后在 $B_4$ 汇合结果。在理想解中，假设$Entry$的分支条件是`if (a * a < 0)`。在实数领域中，这个条件永远为假，所以程序实际上永远只会走$B_2$ （假设$B_1$是条件为真的目的节点）。所以理想解可以描述为$IDEAL[B_4] = f_{B_3}(f_{B_2}(v_{ENTRY}))$ 。
